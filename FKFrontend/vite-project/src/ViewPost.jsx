...
function ViewPost({ onNavigate, post, userType, setSelectedPost, userSchoolId }) {
  if (!post) {
    return (
  ...
  // Ensure tags is an array even if it's a string from backend (e.g., from PHP)
  ...
  const isOriginalPoster = userSchoolId && post.schoolId === userSchoolId;

  return (
    <div className="max-w-2xl mx-auto p-6 bg-blue-50 dark:bg-gray-800">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-2xl font-bold text-blue-600 dark:text-blue-300">View Item</h2>
        <button
          title="Back"
      ...
      <div className="mb-4 text-sm">
        <p className="text-blue-500 dark:text-blue-300 font-semibold">Location Found:</p>
        <p className="text-gray-600 dark:text-gray-200">{post.location || 'Not specified'}</p>
      </div>
      ...
      {userType !== 'admin' && !isOriginalPoster && (
